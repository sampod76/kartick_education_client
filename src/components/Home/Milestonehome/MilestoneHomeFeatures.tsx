...
export default function MilestoneHomeFeatures() {
    ...
    return course?.title && course?.title == claseAcctive
      ...
      course?.course?.courses.map((item: any, index: number) => {
        ...
        return (
          ...
          <div key={index} className="flex gap-3 p-3 border-b-4 justify-center hover:border-b-black transition-all cursor-pointer bg-primary text-white text-center shadow-sm  items-center rounded-full">
            ...
            <Link href={`/comingsoon`}><h2 className="text-base text-center md:text-lg ">{item.title}</h2></Link>
          </div>
  ...
  const activeClass =
    "   text-[14px] lg:text-[18px] font-[600] bg-black text-white";
              ...
              data?.data?.map((category: any, index: number) => {
                return (
                  <li
                    ...
                    className={`${claseAcctive == category.title
                        ? activeClass
                        : inactiveClass
