...
import { useGetAllPurchasePackageQuery, useGetSinglePurchasePackageQuery } from '@/redux/api/public/purchaseAPi';
...
export default function DetailsPurchasePackage({ purchaseId }: { purchaseId: string }) {
    ...
    const { data: allPurchaseData, isLoading } = useGetAllPurchasePackageQuery({ status: "active", limit: 9999999999 })
    ...
    return (
        <div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 ">
                ...
                {/* Package section */}
                <div
                    className="rounded-md   relative bg-blue-200 min-h-full  lg:min-h-[30rem] max-w-2xl lg:max-w-3xl "
                >
                  ...
                  `}
                    ...
                    <div className="">
                        ...
                        <div className="flex justify-between max-w-[80%] mx-auto text-gray-600 py-3">
                            ...
                            <h2 className='text-sm text-start '>Payment Status {packages?.paymentStatus === "approved" ? <button className="text-sm p-1 rounded-sm text-white bg-green-700">Approved</button> : packages?.paymentStatus === 'pending' ? <button className="text-sm p-1 rounded-sm text-white bg-red-600">Pending</button> : <button className="text-sm p-1 rounded-sm text-white bg-red-600">Rejected</button>}</h2>
                ...
                <div className="max-w-2xl lg:max-w-3xl bg-white shadow-lg rounded-lg overflow-hidden ">
                    ...
                    <div className="py-4 px-6">
                        ...
                        <div className="flex items-center mt-4 text-gray-700">
                            ...
                            <h1 className="px-2 text-sm">{packages?.membership?.title}{" (Package )"}</h1>
                        </div>
                        <div className="flex items-center mt-4 text-gray-700">
                            <svg className="h-6 w-6 fill-current" viewBox="0 0 512 512">
